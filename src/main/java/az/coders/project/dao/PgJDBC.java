...
import main.java.az.coders.project.exceptions.DataNotFoundException;
import main.java.az.coders.project.sql.PgSql;
import main.java.az.coders.project.sql.Sql;
...
public class PgJDBC implements JDBC {
    ...
    @Override
    public void updateFlightsDate(SimpleDateFormat format, java.util.Date date) {
        try {
            statement.execute(sql.updateDate(format,date));
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
